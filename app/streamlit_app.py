...
# -----------------------
    ...
    # Optional GPT rationale polish
if st.session_state.dec_json and st.checkbox("Refine rationales with GPT-4.1"):
    try:
        from core.stage3_decision.rationales_gpt import refine_rationales
        # Build signals again for the prompt
    ...
    if st.button('Decide'):
        try:
            dec = decide(signals, policy={"max_position_pct": 5, "stop_loss_pct": 8, "take_profit_pct": 15, "min_confidence": "medium"})
